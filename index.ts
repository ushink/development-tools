...
export function renderFormSelectionLevel(
    appEl: Element | null,
    playGame: (lengthArray: number, appEl: Element) => void,
) {
    const appHTML = `
    <div class="center">
        ...
        document
            ...
            ?.addEventListener('submit', (event) => {
                ...
                let level = Array.from(
                    document.querySelectorAll('.button_level__radio'),
                );

                for (let card of level) {
                    if ((card as HTMLInputElement).checked) {
                        let lengthArray: number = Number(
                            (card as HTMLInputElement).value,
                        );
